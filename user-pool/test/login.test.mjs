...
describe("works with the correct deviceSide and habitType", () => {
  ...
  it("Fails when deviceSide is too low", async () => {
    ...
    expect(JSON.parse(response.body).failure).toEqual(
      "Failure when creating item in HabitEventTable"
    );
  });
...
const resetEvent = () => {
  event = {
    routeKey:
      "PUT /createHabit/{userId}/{deviceId}/{habitName}/{habitType}/{deviceSide}",
    pathParameters: {
      userId: "0",
      deviceId: "TestDevice",
